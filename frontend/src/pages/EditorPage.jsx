...
function FlowContent() {
  ...
  const [chatMessages, setChatMessages] = useState([
    { role: 'assistant', content: '?àÎÖï?òÏÑ∏?? ?åÌÅ¨?åÎ°ú???òÏ†ï???ÑÏ??úÎ¶¥ÍπåÏöî? ?êÌïò?úÎäî Íµ¨ÏÑ±??ÎßêÏ???Ï£ºÏÑ∏?? (?? ?¥Î©î???ÑÏÜ° ?∏ÎìúÎ•?Ï∂îÍ??òÍ≥† ?¨Îûô ?åÎ¶º???∞Í≤∞?¥Ï§ò)' }
  ]);
  ...
  useEffect(() => {
    if (projectId) {
      ...
      if (location.state?.prompt) {
        setProjectTitle("AI ?ùÏÑ± ?åÌÅ¨?åÎ°ú??);
      }
  ...
  const handleSave = async () => {
    try {
      ...
      alert("?ÑÎ°ú?ùÌä∏ ?Ä?•Ïóê ?§Ìå®?àÏäµ?àÎã§.");
      return false;
    }
  };
  ...
  const handleOpenDeployModal = async () => {
    if (!currentId) {
      alert("Î®ºÏ? ?ÑÎ°ú?ùÌä∏Î•??Ä?•Ìï¥ Ï£ºÏÑ∏??");
      return;
    }
  ...
  const handleSendChat = () => {
    ...
    setTimeout(() => {
      setChatMessages(prev => [
        ...prev, 
        { role: 'assistant', content: '?îÏ≤≠?òÏã† ?¥Ïö©??Î∂ÑÏÑù?òÏó¨ ?åÌÅ¨?åÎ°ú???ÖÎç∞?¥Ìä∏Î•?Ï§ÄÎπ?Ï§ëÏûÖ?àÎã§. (?ÑÏû¨ UI ?åÏä§??Î™®Îìú?ÖÎãà?? Ï∂îÌõÑ LLM API ?∞Îèô???ÑÏöî?©Îãà??)' }
      ]);
    }, 1000);
  };
    ...
    <div className="app-container">
            ...
            {isDrawerOpen && (
              <div style={{ 
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '0.3rem' }}>?ÑÎ°ú?ùÌä∏ ?úÎ™©</label>
                  <input 
                    type="text" 
                    value={projectTitle} 
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '0.3rem' }}>?ÑÎ°ú?ùÌä∏ Î™ÖÏÑ∏ (Description)</label>
                  <textarea 
                    value={projectDescription} 
                    onChange={(e) => setProjectDescription(e.target.value)}
                    disabled={!isOwner}
                    rows={4}
                    placeholder="???åÌÅ¨?åÎ°ú?∞Ïóê ?Ä???§Î™Ö?¥ÎÇò Í∏∞Ìöç ?òÎèÑÎ•??ÅÏñ¥?êÏÑ∏??"
                    style={{ 
                      width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-color)', 
                      color: 'var(--text-color)', fontSize: '0.9rem', padding: '0.5rem', borderRadius: '4px', outline: 'none',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          {isOwner && (
            <>
              ...
              >
                <Share2 size={16} />
                {isPublic ? 'Í≥µÍ∞ú?? : 'ÎπÑÍ≥µÍ∞?}
              </button>
              <button className="btn-secondary" onClick={() => { handleSave().then(s => s && alert("?Ä?•Îêò?àÏäµ?àÎã§.")); }}>
                <Save size={16} />
                ?Ä??
              </button>
              {currentId && (
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h3 style={{ margin: 0, fontSize: '1rem', color: 'var(--text-color)', fontWeight: 600 }}>AI ?åÌÅ¨?åÎ°ú???¥Ïãú?§ÌÑ¥??/h3>
        </div>
        ...
        <div style={{
          ...
          <input
            ...
            placeholder="AI?êÍ≤å ?òÏ†ï?¨Ìï≠???îÏ≤≠?òÏÑ∏??.."
            style={{
              flex: 1,
