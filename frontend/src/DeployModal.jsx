  ...
  return (
    <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="modal-content" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '12px', width: '500px', maxWidth: '90vw', padding: '1.5rem', position: 'relative' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: '1.5rem', right: '1.5rem', background: 'transparent', border: 'none', color: '#94a3b8', cursor: 'pointer' }}>
          <X size={20} />
        </button>
        
        <h2 style={{ margin: '0 0 1.5rem 0', color: 'white', fontSize: '1.3rem' }}>워크플로우 배포</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '2rem' }}>
          ...
          >
            <Bot size={32} color={deployMode === 'chatbot' ? '#3b82f6' : '#94a3b8'} style={{ margin: '0 auto 0.5rem' }} />
            <h3 style={{ margin: '0 0 0.5rem', fontSize: '1rem', color: 'white' }}>Web App (Chatbot)</h3>
            <p style={{ margin: 0, fontSize: '0.8rem', color: '#94a3b8' }}>대화형 챗봇 인터페이스로 배포합니다.</p>
          </div>
          ...
          >
            <LayoutTemplate size={32} color={deployMode === 'form' ? '#10b981' : '#94a3b8'} style={{ margin: '0 auto 0.5rem' }} />
            <h3 style={{ margin: '0 0 0.5rem', fontSize: '1rem', color: 'white' }}>Web App (Form)</h3>
            <p style={{ margin: 0, fontSize: '0.8rem', color: '#94a3b8' }}>단순한 입력 폼과 버튼 형태의 페이지로 배포합니다.</p>
          </div>
          ...
          >
            <Code2 size={32} color={deployMode === 'fastapi' ? '#eab308' : '#94a3b8'} style={{ margin: '0 auto 0.5rem' }} />
            <h3 style={{ margin: '0 0 0.5rem', fontSize: '1rem', color: 'white' }}>FastAPI Server</h3>
            <p style={{ margin: 0, fontSize: '0.8rem', color: '#94a3b8' }}>독립적인 백엔드 서버 파이썬 스크립트를 다운로드합니다.</p>
          </div>
          ...
          >
            <Download size={32} color={deployMode === 'mcp' ? '#8b5cf6' : '#94a3b8'} style={{ margin: '0 auto 0.5rem' }} />
