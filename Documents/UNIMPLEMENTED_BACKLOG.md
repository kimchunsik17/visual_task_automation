...
> **2026-09-06 갱신 — 이 색인은 08-30 기준이다.** 그 뒤 `ROADMAP.md` 가 v3.0 으로 재작성되면서 백로그
> 32~38번(실행 엔진 v2·앱 빌더 통합·개발 도구 노드·흐름 제어 보완·에이전트/MCP·운영·의존성 맵)이 생겼고,
> 그것들은 **여기 없다** — ROADMAP §2·§3 이 정본이다. 이 문서의 항목 중 상태가 바뀐 것:
>
> - **T1**(템플릿의 일반 필드 선언 불가) → ROADMAP 33번 **APP-0 사용자 제공 필드 스키마**가 흡수했다.
> - **Q3**(미완성 노드 3종 등급) → 제품 판단은 미결 그대로. 시연 기간에는 `HIDDEN_NODE_TYPES` 플래그로 숨길 수
>   있게 됐으나 **현재 목록은 `jusoNode` 하나**다.
> - **Q9**(승인키) → 도로명주소는 시연에서 제외했다. 실호출 대조는 여전히 남았다(ROADMAP §3.14).
> - **Q10**(`--accent-color`) → 08-30 에 이미 해결됐다(`index.css:21`). 이 문서가 낡아 있었다.
> - **Q20**(포인팅) → 재개 조건을 "도구 단위 제약" 으로 정했고 36번 AGENT-0 이 그 부품이다(ROADMAP §3.10).
> - **Q6**(로컬 LLM 검증) → PICKLE 게이트웨이 전환(09-05)으로 우선순위가 내려갔다.
> - §2.1 Q2·§2.4 Q11~Q16·§2.5 Q17~Q19 는 변동 없음.
