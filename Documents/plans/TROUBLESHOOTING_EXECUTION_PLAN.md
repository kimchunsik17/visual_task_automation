...
> ⚠️ **먼저 [TROUBLESHOOTING_REVERIFICATION.md](TROUBLESHOOTING_REVERIFICATION.md) 를 읽어라 (2026-09-01).**
> 이 문서의 실측치는 2026-08-31 기준이고 그 뒤 PR #24~#31 이 머지됐다. 재검증에서 **개수 주장
> 5건이 과소 추정**으로 드러났고(alert 22→78, 미컴파일 7→16, log_step 미호출 3→6, 오염 경로
> 2→4), **줄 번호가 12곳 표류**했으며, **"일부러 하지 않는 것" 중 2개는 전제가 무너져 결정을
> 다시 봐야 한다**(SSRF·드리프트 테스트). 0단계 일부와 `/api/runs` fail-open 은 이미 처리됐다.
