...
| [ADR.md](ADR.md) | 결정 기록 | "왜 이렇게 만들었나"를 물을 때. ADR-0001~0029 |
...
**브랜치는 셋뿐이다 — `main`, `dev`, 그리고 짧은 작업 브랜치.** (`dev` 는 2026-09-06 부터)

| 브랜치 | 뜻 | 누가 커밋하나 |
| --- | --- | --- |
| `main` | **서버에 배포된 상태.** `scripts/deploy.sh` 가 배포하는 것이 이 브랜치다 | 직접 커밋 없음. `dev` 에서 **배포할 때** merge commit 으로 합친다 |
| `dev` | **통합 브랜치.** 다음 배포에 나갈 것이 모인다 | 직접 커밋 없음. 작업 브랜치의 PR 만 받는다 |
| `feat/…`·`fix/…`·`docs/…` | **작업 단위 하나** — 계획서의 한 Phase, 버그 하나, 안정화 항목 하나 | `dev` 에서 따고, 끝나면 `dev` 로 PR, 합친 뒤 지운다 |

`dev` 를 둔 이유: 시연회(2026-09-09~11) 동안 `main` 은 부스에 배포된 상태로 묶여 있어야 하는데 개발은
멈추지 않기 때문이다. 시연 뒤에도 유지한다 — "`main` = 지금 서버에 있는 것" 이 항상 참이면 장애 때
`rollback.sh` 의 기준점이 명확하다.

- `dev` → `main` 은 **배포하는 사람이, 배포 직전에** 합친다. 합친 뒤 `deploy.sh`.
- 배포가 급한 수정(hotfix)만 예외다 — `main` 에서 따서 `main` 으로 PR 하고, **같은 날 `dev` 에도 합친다.**
  안 합치면 다음 배포에서 수정이 되돌아간다.
- `dev` 가 `main` 보다 뒤처졌으면(hotfix 뒤) `dev` 에서 `git merge main`. 반대 방향 rebase 는 하지 않는다.

**아래 08-31 사고와 다른 점.** 그때의 문제는 장기 브랜치에 **커밋되지 않은** 7일치가 쌓인 것이었다. `dev` 는
직접 커밋을 받지 않고 PR 만 받으므로 모든 변경이 리뷰 가능한 단위로 남는다. `dev` 에 직접 커밋하기
시작하면 같은 사고다.

2026-08-31 에 짧은 브랜치 규칙을 세운 이유가 있다. 마지막 커밋이 08-24 였고 7일치(336파일)가
