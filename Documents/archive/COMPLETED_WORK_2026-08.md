...
| 분리일 | 2026-08-30 |
| 출처 | `LONG_TERM_PRODUCT_ROADMAP.md` v1.9 (원본은 `archive/LONG_TERM_PRODUCT_ROADMAP_v1.9.md`) |
| 현재 로드맵 | `../ROADMAP.md` |

2026-08-26부터 08-29까지 진행한 작업의 설계 근거와 구현 기록이다. 로드맵에서 잘라 왔고,
내용은 그대로다. 왜 그렇게 만들었는지 다시 볼 때만 열면 된다.

## 완료 요약

| 백로그 | 작업 | 완료일 | ADR | 마이그레이션 |
| ---: | --- | --- | --- | --- |
| 1 | Node Definition v1과 3개 노드 vertical slice | 2026-08-28 | ADR-0005 | — |
| 2 | ProjectRevision과 Alembic | 2026-08-28 | ADR-0006 | 0001 |
| 3 | 과업형 튜토리얼 MVP (첫 두 slice) | 2026-08-26 | — | — |
| 4 | 공통 Connector/OAuth/Error 계약 | 2026-08-28 | ADR-0007 | — |
| 5 | Node Knowledge Index와 hybrid retrieval | 2026-08-28 | ADR-0013 | — |
| 6 | YouTube Trigger/Action vertical slice | 2026-08-28 | ADR-0008 | — |
| 7 | Mock 탭 webhook vertical slice | 2026-08-28 | ADR-0009 | — |
| 8 | RSS·Gmail·Drive Wave 1 | 2026-08-28 | — | — |
| 9 | Inspector로 주요 10개 노드 이전 | 2026-08-28 | ADR-0005 | — |
| 10 | GenerationPlan·adaptive candidate | 2026-08-28 | — | — |
| 12 | 커뮤니티 템플릿 승격 | 2026-08-29 | ADR-0023 | 0014 |
| 15~16 | App Builder 값 저장소·Submit/Output | 2026-08-28 | ADR-0011 | — |
| 17 | 업로드 인증·quota·TTL·경로 검증 | 2026-08-28 | ADR-0010 | — |
| 18 | App Builder 파일 컴포넌트 | 2026-08-28 | ADR-0012 | — |
| 19 | Database Query PostgreSQL vertical slice | 2026-08-28 | ADR-0017 | 0009 |
| 20 | Artifact 기반 Discord·Email 파일 전송 | 2026-08-29 | ADR-0018 | 0010 |
| 21 | NodeError v1과 공통 오류 catalog | 2026-08-28 | ADR-0016 | 0008 |
| 22 | 커뮤니티 안전·정체성 공통 기반 | 2026-08-29 | ADR-0020 | 0011 |
| 23 | 커뮤니티 Q&A MVP | 2026-08-29 | ADR-0021 | 0012 |
| 24 | 사용자 간 메시지 MVP (친구 한정) | 2026-08-29 | ADR-0022 | 0013 |
| 25 | pythonNode 실행 격리와 자원 한도 | 2026-08-29 | ADR-0019 | — |
| 11 | Workspace/RBAC — **TEAM-0·1만** | 2026-08-29 | ADR-0024 | 0015 |

이 밖에 `INCOMPLETE_NODE_STRUCTURE_REVIEW.md`의 P0(ADR-0014)와 사용자 승인 durable 대기
(ADR-0015)도 2026-08-28에 완료했다.

### 완료했지만 결론이 "하지 않는다"인 것

- **10번 adaptive fan-out 기본값 전환** — 2라운드 비교에서 지연 +58%에 품질 이득이 재현되지
  않아 §8 중단 기준을 적용해 미채택으로 확정했다. 인프라와 계측은 유지한다.
- **`transformNode` 도입** — `pythonNode` 저장 코드의 유형 분포에서 뚜렷한 상위 유형이 나오지
  않아 만들지 않기로 했다.

### 완료했지만 사용자 설정을 기다리는 것

- 실제 Google credential(YouTube·Gmail·Drive) end-to-end 검증 — Google Cloud 프로젝트와
  OAuth 동의 화면이 사용자 계정에 묶여 있다.
- 실제 Discord·Gmail credential로 하는 첨부 전송 확인.

---

## 4.1 팀 단위 작업 — ProjectRevision·Alembic
...
> **완료 메모.** 백로그 2번. ADR-0006. 이 절의 팀 데이터 모델 제안은 현행 로드맵 §3.1(Workspace/RBAC)이 대체했다.
...
> **완료 메모.** 백로그 1·9번. ADR-0005. 정의 기반 노드 15종.
...
> **완료 메모.** 백로그 10번. 인프라는 완료, 기본값 전환은 2라운드 비교 끝에 **미채택 확정**.
...
> **완료 메모.** 백로그 3번. 첫 두 vertical slice 완료(2026-08-26).
...
> **완료 메모.** 백로그 7번. ADR-0009.

---

## 4.7-A LLM 생성 기능 강화: Hybrid Node RAG
...
> **완료 메모.** 백로그 5번. ADR-0013. hybrid Recall@10 100%. 기본 selector 승격(RAG Phase C)은 운영 shadow 비교 대기.

---

## 4.7-B Node RAG 구현 진행 상황
...
## 4.7-C YouTube vertical slice와 Wave 1(RSS·Gmail·Drive)
...
> **완료 메모.** 백로그 6·8번. ADR-0007·0008. 실제 Google credential 검증만 사용자 설정 대기.
...
> **완료 메모.** 백로그 15~18번. ADR-0010·0011·0012.
...
> **완료 메모.** 백로그 19번. ADR-0017. MySQL과 pool hardening(DB-4)은 게이트 뒤로 남겼다.
...
> **완료 메모.** 백로그 20번. ADR-0018. 실제 credential end-to-end 확인만 사용자 설정 대기.
...
> **완료 메모.** 백로그 21번. ADR-0016. 나머지 노드 이전은 legacy 비율 telemetry를 보며 점진 진행.
...
> **완료 메모.** 백로그 23번. ADR-0021. COMMUNITY-4(검수 화면) 포함.
...
> **완료 메모.** 백로그 25번. ADR-0019. `transformNode` 도입은 표본 부족으로 보류.
