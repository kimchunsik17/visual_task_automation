...
# outputNode 는 return 으로, humanApprovalNode 는 승인/거절 분기로 제어를 끊거나 나눈다 — 본문만 떼어 낼 수
# 없어 인터프리터가 직접 구현한다. 구조 앞의 곧은 줄과 판정 규칙은 여기 함수로 두 엔진이 공유한다.

# 승인으로 인정하는 결정 값. 생성 코드에는 repr 로 박히고 인터프리터는 is_approved 로 판정한다.
APPROVAL_YES_VALUES = ['Y', 'YES', 'APPROVE', 'TRUE', '1']


def is_approved(decision):
    return str(decision).strip().upper() in APPROVAL_YES_VALUES


def emit_output_body(lines, node_id, node, indent, prev_res_var):
...
def approval_targets(node_id, forward_edges):
    """(approved, rejected, plain) 대상 목록. rejected 는 'rejected' 와 'else' 핸들, plain 은 핸들 없는 옛 연결."""
    ...
    return approved_edges, rejected_edges, plain_edges


def emit_approval_header(lines, node_id, node, indent, prev_res_var):
    """결정 읽기(없으면 durable 대기 신호) · 입력 통과 · 기록 — 분기 구문 앞의 곧은 줄."""
...
def generate_output_node(node_id, node, indent, active_llm_id, prev_res_var, visited, node_dict, forward_edges, incoming_edges, lines, generate_block_fn):
    emit_output_body(lines, node_id, node, indent, prev_res_var)
...
def generate_human_approval_node(node_id, node, indent, active_llm_id, prev_res_var, visited, node_dict, forward_edges, incoming_edges, lines, generate_block_fn):
    ...
    emit_approval_header(lines, node_id, node, indent, prev_res_var)

    approved_edges, rejected_edges, plain_edges = approval_targets(node_id, forward_edges)
    ...
    if approved_edges or rejected_edges:
        lines.append(f"{indent}if str(approval_{node_id}).strip().upper() in {APPROVAL_YES_VALUES!r}:")
        ...
        lines.append(f"{indent}if str(approval_{node_id}).strip().upper() not in {APPROVAL_YES_VALUES!r}:")
        lines.append(f"{indent}    raise Exception('Workflow execution halted by Human Approval Node (Rejected).')")
