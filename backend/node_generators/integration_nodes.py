...
def generate_discord_node(node_id, node, indent, active_llm_id, prev_res_var, visited, node_dict, forward_edges, incoming_edges, lines, generate_block_fn):
    ...
    lines.append(f"{indent}                f'https://discord.com/api/v10/channels/{{discord_channel_{node_id}.strip()}}/messages',")
    lines.append(f"{indent}                headers={{'Authorization': f'Bot {{discord_token_{node_id}.strip()}}', 'Content-Type': 'application/json'}},")
