...
DEFAULT_RULES = {
    ...
    # 인바운드 웹훅 엔드포인트별 분당 상한(백로그 34 DEV-0). 주체는 사용자가 아니라 **엔드포인트(프로젝트)** — 공개 URL 이라
    # 누가 부르는지 모른다. GitHub 이 push 마다 이벤트 하나를 보내므로 분당 120 은 활발한 저장소 여러 개를 덮는다.
    # 조정: RATE_LIMIT_WEBHOOK_RECEIVE.
    "webhook.receive": (120, 60),
}
