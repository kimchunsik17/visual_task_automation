...
def get_llm(provider: str = "openai"):
    if provider == "gemini":
        return ChatGoogleGenerativeAI(model="gemini-1.5-pro", temperature=0.1).with_structured_output(AppGeneratorResult)
    else:
        return ChatOpenAI(model="gpt-4o", temperature=0.1).with_structured_output(AppGeneratorResult)

async def generate_app(prompt: str, current_state: dict, provider: str = "openai") -> AppGeneratorResult:
